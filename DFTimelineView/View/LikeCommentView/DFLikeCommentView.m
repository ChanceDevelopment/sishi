...
{
    MLClickColorLinkLabel *currentSelectLabel;
}
...
@property (assign, nonatomic) BOOL menuItemShowing;
    ...
    if (_likeLabel == nil) {
        ...
        [_likeLabel setDidClickLinkBlock:^(MLLink *link, NSString *linkText, MLLinkLabel *label) {
            ...
            if (_delegate != nil && [_delegate respondsToSelector:@selector(onClickUser:)]) {
                ...
                NSString *userId = link.linkValue;
...
{
    NSLog(@"deleteMenuAction");
    _menuItemShowing = NO;
    NSString *uniqueId = currentSelectLabel.uniqueId;
    NSLog(@"uniqueId = %@",uniqueId);
    if (_delegate && [_delegate respondsToSelector:@selector(onClickComment:)]) {
        [_delegate onDeleteCommentWithCommentID:uniqueId];
    }
}
...
{
    _menuItemShowing = NO;
    NSLog(@"copyMenuAction");
    NSRange range = [currentSelectLabel.attributedText.string rangeOfString:@" "];
    NSString *substring = currentSelectLabel.attributedText.string;
    if (range.length != 0) {
        @try {
            substring = [currentSelectLabel.attributedText.string substringFromIndex:range.location + range.length];
        } @catch (NSException *exception) {
            substring = currentSelectLabel.attributedText.string;
        } @finally {
    ...
    NSLog(@"copyText = %@",pasteboard.string);
}
...
- (BOOL)isEditMenuShowing
{
    return _menuItemShowing;
}
...
{
    NSLog(@"长按了Label");
    _menuItemShowing = YES;
    [self becomeFirstResponder];
    currentSelectLabel = (MLClickColorLinkLabel *)recognizer.view;
    ...
    [_menuController setMenuItems:@[deleteMenuItem,copyMenuItem]];
    [_menuController setTargetRect:currentSelectLabel.frame inView:currentSelectLabel.superview];
    [_menuController setMenuVisible:YES animated:YES];
}

-(void)onClickOutsideLink:(NSString *)uniqueId
{
    NSLog(@"单击了Label: %@", uniqueId);
